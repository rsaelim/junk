...
// Idea for optimizations:
    ...
    Unecessary work: Everything before the pointers should be somewhat matching 
...
public:
    int equalSubstring(string s, string t, int maxCost) {
        ...
        int maxLength = 0;
        int curCost = abs( s[0] - t[0] );
        int left = 0, right = 0;
        
        // Update max
        if ( (right - left + 1) > maxLength && curCost <= maxCost ) 
            maxLength = ( right - left + 1); 
        
        while( right < s.length() ) { // loop
         
            if ( curCost > maxCost ) {
                
                if ( left == right ) { // adjust cost, increment both pointers
                    curCost -= abs( s[ right ] - t[ right ] );
                    right++;
                    curCost += abs( s[ right ] - t[ right ] );
                }
                
                else // adjust cost, incr left
                    curCost -= abs( s[ left ] - t[ left ] );
                
                left++;
            }
            else { // incr right, adjust cost
                right++;
                curCost += abs( s[ right ] - t[ right ] );
            }
            ...
            // If we are pointing at the last element, our maxLenght cannot be greater
            if ( right == s.length() - 1 )
                return maxLength;
        }
...
class Node {
    ...
    Node( string value ) : val( value ), isEnd( true )
    {
    }
...
void addToRes( vector<string> &res, Node* head ) {
    for (auto itr = head->children.begin(); itr != head->children.end(); ++itr ) {
        if ( itr->second->isEnd )
            res.push_back( itr->second->val );
        else
            addToRes( itr->second );
    }
}
...
vector<string> removeSubfolders(vector<string>& folder) {
    ...
    for ( string folder : folders ) {
        ...
        for ( int i = 1; i < folder.length(); ++i ) {
            if ( folder[i] != '/' ) { subPath += folder[i]; }
            else if ( subPath != "" ) {
                if ( itr->children.find( subPath ) != itr->children.end() ) // if subPath exists
                    if ( itr->isEnd ) { break; } // if this is end of path, we have no luck
                else
                    itr->children[ subPath ] = new Node( suBpath ); // if subPath does not exist, we create
                itr = itr->children[ subPath ]; // reset itr to point to new subPath
                subPath = ""; // reset
            }
        } // for subPath
        itr->isEnd = true;
